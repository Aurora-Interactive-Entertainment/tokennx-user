...
审计材料在已有 `.gitignore` 忽略的目录，不进入默认测试集合。认证与企业测试采用“断言缺陷确实发生”的写法，因此通过意味着复现成功；生成测试采用“断言正确行为”的写法，因此当前失败意味着揭示了缺陷。两者都不改变原有 853 个测试全部通过的事实。

- [认证复现脚本](D:/Project/tokennx-user/.tmp-audit-auth-20260916/reproduce-auth-issues.mjs)及[输出](D:/Project/tokennx-user/.tmp-audit-auth-20260916/reproduction-output.json)：7 个合成场景，无真实网络。
- [企业与支付复现配置](D:/Project/tokennx-user/.tmp-audit-billing-enterprise-20260916/vitest.config.ts)及[输出](D:/Project/tokennx-user/.tmp-audit-billing-enterprise-20260916/results.json)：7 个用例成功复现企业购买、关单异常、角色、部门、错误态、晚付款和切换企业余额问题。
- [模型与视频子报告](D:/Project/tokennx-user/.tmp-audit-runtime-20260916/report.md)：4 个针对性失败用例及静态调用链说明。
