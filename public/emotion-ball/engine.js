...
(function () {
  ...
  /* 计算双眼眼环的水平中心；部分原始眼型把注视方向直接烘焙进了坐标。 */
  function ringPairCenterX(pair) {
    var sum = 0, count = 0;
    for (var eye = 0; eye < pair.length; eye++) {
      for (var i = 0; i < pair[eye].length; i++) {
        sum += pair[eye][i][0];
        count++;
      }
    }
    return count ? sum / count : RD.HEAD_C;
  }

  /* 把左眼轮廓平移到右眼中心，保留相同形状与填充面积，避免跟随时出现大小眼。 */
  function matchRingGeometry(source, target) {
    var sourceX = 0, sourceY = 0, targetX = 0, targetY = 0;
    ...
    return source.map(function (point) {
      return [point[0] - sourceX + targetX, point[1] - sourceY + targetY];
    });
  }
  ...
  function Engine(target, opts) {
    ...
    /* 小尺寸入口可单独调节注视幅度和响应速度，默认值保持展馆原有观感。 */
    this._gazeScale = clamp(opts.gazeScale == null ? 1 : opts.gazeScale, 0.6, 1.8);
    this._gazeSpeed = clamp(opts.gazeSpeed == null ? 5.66 : opts.gazeSpeed, 3, 18);
    this._gazeUpScale = clamp(opts.gazeUpScale == null ? 1 : opts.gazeUpScale, 0.6, 1);
    this._gazeDownScale = clamp(opts.gazeDownScale == null ? 1 : opts.gazeDownScale, 1, 1.6);
    this._gazeCenterY = clamp(opts.gazeCenterY == null ? 0 : opts.gazeCenterY, 0, 16);
    this._gazeEyeScale = clamp(opts.gazeEyeScale == null ? 1 : opts.gazeEyeScale, 1, 1.25);
    this._equalizeEyeSize = opts.equalizeEyeSize === true;
    /* 注视接管仅按实例开启；鼠标停下后自动淡回表情自身的眼神动画。 */
    this._gazeOverride = opts.gazeOverride === true;
    this._gazeIdleDelay = clamp(opts.gazeIdleDelay == null ? 0 : opts.gazeIdleDelay, 0, 5000);
    this._gazeActiveUntil = 0;
    this._gazeBlend = 0;
    this._lastTick = 0;
  ...
  Engine.prototype = {
    ...
    /* 注视目标：横向 ±24、纵向 ±15（viewBox 坐标），可按实例做轻微放大 */
    setGaze: function (nx, ny) {
      this._gaze.tx = clamp(nx, -1, 1) * 24 * this._gazeScale;
      var verticalScale = ny > 0 ? this._gazeDownScale : this._gazeUpScale;
      this._gaze.ty = clamp(ny, -1, 1) * 15 * this._gazeScale * verticalScale;
      if (this._gazeOverride) {
        this._gazeActiveUntil = performance.now() + this._gazeIdleDelay;
        /* 鼠标接管时固定到当前表情的主眼型，停止左右眼型轮换造成的反向错觉。 */
        if (this._def && this._def.pool.length) {
          this._poolPos = 0;
          if (this._exprIdx !== this._def.pool[0]) this._setExpr(this._def.pool[0], 18);
        }
      }
      return this;
    },
    ...
    _compose: function (now, depth) {
      ...
      var pose;
      var gazeFollowing = this._gazeOverride && def.gaze !== false && now < this._gazeActiveUntil;
      ...
      for (var i = 0; i < def.anims.length; i++) {
        var anim = def.anims[i];
        /* 鼠标活动期间只暂停自主左右/上下扫视，其余呼吸、弹跳等原动效继续运行。 */
        if (gazeFollowing && anim.target === 'eyes' && (anim.prop === 'lookX' || anim.prop === 'lookY')) continue;
        applyAnim(pose, anim, t, this);
      }
      ...
      if (this._active && !gazeFollowing && now >= this._poolNext) {
      ...
      /* 自主动画保留原始双眼；鼠标接管时统一轮廓几何，位置仍使用各自眼位。 */
      if (gazeFollowing && this._equalizeEyeSize) {
        pose.right.ring = matchRingGeometry(pose.left.ring, pose.right.ring);
      }

      /* 鼠标注视：帧率无关的指数平滑，小尺寸入口可提高速度以避免跟手感被待机动画掩盖。 */
      var k = 1 - Math.exp(-this._gazeSpeed * dt);
      ...
      var idleGazeWeight = 1;
      if (this._gazeOverride) {
        /* 鼠标活跃时平滑接管原有扫视，停止后较慢淡出，避免两套眼神互相拉扯。 */
        var followTarget = gazeFollowing ? 1 : 0;
        var blendSpeed = followTarget ? 18 : 4.5;
        var blendK = 1 - Math.exp(-blendSpeed * dt);
        this._gazeBlend += (followTarget - this._gazeBlend) * blendK;
        idleGazeWeight = 1 - this._gazeBlend;
        /* 先消除眼型数据自带的左右偏移，再叠加鼠标方向，保证左移必定看左、右移必定看右。 */
        var centeredGazeX = this._gaze.x + RD.HEAD_C - ringPairCenterX(this._ringCur);
        pose.left.lookX = pose.left.lookX * idleGazeWeight + centeredGazeX * this._gazeBlend;
        pose.right.lookX = pose.right.lookX * idleGazeWeight + centeredGazeX * this._gazeBlend;
        var centeredGazeY = this._gaze.y + this._gazeCenterY;
        pose.left.lookY = pose.left.lookY * idleGazeWeight + centeredGazeY * this._gazeBlend;
        pose.right.lookY = pose.right.lookY * idleGazeWeight + centeredGazeY * this._gazeBlend;
      } else {
      ...
      if (def.gaze !== false) {
        var w = now / 1000;
        pose.left.lookX += idleGazeWeight * (1.4 * Math.sin(0.42 * w) + 0.5 * Math.sin(1.0 * w));
        pose.right.lookX += idleGazeWeight * (1.4 * Math.sin(0.42 * w + 1) + 0.5 * Math.sin(1.0 * w + 2));
        pose.left.lookY += idleGazeWeight * 0.9 * Math.sin(0.58 * w);
        pose.right.lookY += idleGazeWeight * 0.9 * Math.sin(0.58 * w + 1);
      }

      /* 鼠标注视到球面边缘时会产生透视压缩，按接管权重轻微补偿双眼尺寸。 */
      var gazeEyeScale = 1 + (this._gazeEyeScale - 1) * this._gazeBlend;
      pose.left.scaleX *= gazeEyeScale;
      pose.left.scaleY *= gazeEyeScale;
      pose.right.scaleX *= gazeEyeScale;
      pose.right.scaleY *= gazeEyeScale;
      ...
      /* 交给 SVG 投影层按眼型边界和球面透视统一双眼最终可见尺寸。 */
      pose.eyeSizeBalance = this._equalizeEyeSize ? 1 : 0;
      return pose;
    },
