...
(function () {
  ...
  function createBall(container, opts) {
    ...
    /* ---- 眼睛：轮廓环形变 + 球面投影 ---- */
    function setEye(eye, pose, k, sketch, yaw, matchSize, sizeBalance) {
      ...
      var scaleX = sxBase * cn;
      var scaleY = sy * fy;
      var bounds = ringSize(eye.ring);

      /* 按眼环真实边界与球面投影后的尺寸匹配双眼，避免外侧眼睛被压得更小。 */
      if (matchSize && sizeBalance > 0) {
        var width = Math.max(0.01, bounds.width * Math.abs(scaleX));
        var height = Math.max(0.01, bounds.height * Math.abs(scaleY));
        var matchX = clamp(matchSize.width / width, 0.5, 2);
        var matchY = clamp(matchSize.height / height, 0.5, 2);
        scaleX *= 1 + (matchX - 1) * sizeBalance;
        scaleY *= 1 + (matchY - 1) * sizeBalance;
      }

      eye.node.setAttribute('transform',
        'translate(' + r2(ex) + ' ' + r2(ey0) + ')' +
        (pose.rotate ? ' rotate(' + r2(pose.rotate) + ')' : '') +
        ' scale(' + r2(scaleX) + ' ' + r2(scaleY) + ')' +
    ...
    function applyPose(pose) {
      ...
      var yaw = b.yaw || 0;
      var leftSize = setEye(eyeL, pose.left, 0, sketch, yaw, null, 0);
      setEye(eyeR, pose.right, 1, sketch, yaw, leftSize, pose.eyeSizeBalance || 0);
