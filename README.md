...
前端版本守卫在构建时内联到 HTML，页面处于前台时每 30 秒检查 `/version.json`，首次访问、返回标签页、页面恢复及站内跳转也会触发检查；协议页跳转使用更短的检查间隔。发现新版本后，页面安全空闲时自动刷新；操作进行中、存在未提交草稿或可见弹窗时延期并显示更新提示，待操作完成、草稿清空或弹窗关闭后重新判断是否可以刷新。自动重试有 60 秒冷却，授权回调禁止刷新，避免重复消费一次性授权码。该机制不能替代 CDN 的不缓存规则；开发服务器不启用，需通过生产构建预览验证。
