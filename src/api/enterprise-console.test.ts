...
describe('企业控制台 API 客户端', () => {
  ...
  it('按接口契约发送成员、标签、申请和邀请写请求', async () => {
    ...
    await createEnterpriseInvitation(CONTEXT, { role: 'member', max_uses: 10, expires_at: Date.parse('2026-08-01T00:00:00.000Z'), department_id: 'dept-sales' }, { accessToken: 'token' })
    ...
    expect(requests[8]?.body).toEqual({ role: 'member', max_uses: 10, expires_at: Date.parse('2026-08-01T00:00:00.000Z'), department_id: 'dept-sales' })
