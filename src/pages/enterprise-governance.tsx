...
function RoleList({ roles, selectedID, canCreate, onSelect, onCreate }: { roles: EnterpriseRole[]; selectedID: string; canCreate: boolean; onSelect: (role: EnterpriseRole) => void; onCreate: () => void }) {
  ...
  return <aside className="enterprise-governance-rail" aria-label={t('console.enterprise.governance.roleList')}>
    ...
    {roles.length === 0 ? <EnterpriseEmpty title={t('console.enterprise.governance.noRoles')} description={t('console.enterprise.governance.noRolesHint')} /> : <div className="enterprise-governance-rail-list">{roles.map((role) => { const displayRoleName = roleLabel(role.code, [{ code: role.code, name: role.name, owner_role: role.owner_role }]); return <button type="button" className={`enterprise-governance-rail-item${selectedID === role.id ? ' is-selected' : ''}`} aria-current={selectedID === role.id} key={role.id} onClick={() => onSelect(role)}><span className="enterprise-governance-rail-avatar" aria-hidden="true">{displayRoleName.slice(0, 1)}</span><span className="enterprise-governance-rail-copy"><span className="enterprise-governance-rail-name">{displayRoleName}</span><span className="enterprise-governance-rail-meta">{roleCountLabel(role, t)}</span></span></button> })}</div>}
  </aside>
}
