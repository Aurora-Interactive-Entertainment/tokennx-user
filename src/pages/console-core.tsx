...
export function PlaygroundPage() {
  ...
  async function runTest(promptValue = prompt, replaceAttemptValue = editingAttemptId): Promise<void> {
    ...
    const replacementIndex = replacingAttemptId && selectedSession
      ...
      : -1
    // 编辑或重试只携带目标消息所在分段的上下文，不能恢复之前已清除的内容。
    const replacementContextStart = replacementIndex >= 0
      ? selectedSession?.contextBreaks?.filter((index) => index <= replacementIndex).at(-1) ?? 0
      : 0
    const requestContextMessages = replacementIndex >= 0
      ? (selectedSession?.messages ?? []).slice(replacementContextStart, replacementIndex)
