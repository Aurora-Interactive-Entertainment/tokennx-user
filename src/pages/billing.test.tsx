...
describe('用户费用管理页面', () => {
  ...
  it('展示参考页的费用分析结构并按当前个人账务主体请求', async () => {
    ...
    // 普通账本保持精简，Request ID 只在按请求定位的详情入口展示。
  ...
  it('展示发票概览、FAQ 和历史，并完成三步申请提交', async () => {
    ...
    expect(screen.getByRole('heading', { name: '常见开票问题' })).toBeVisible()
    expect(screen.queryByRole('button', { name: /常见开票问题/ })).not.toBeInTheDocument()
    expect(screen.getByText('发票将在申请提交后 24 小时内处理，填写接收邮箱后会同步发送邮件。发票抬头和税号提交后如需修改，需要重新申请。')).toBeVisible()
  ...
  it('通过认证请求下载已开具发票', async () => {
    ...
    expect(toast).toHaveTextContent('费用管理服务暂时不可用')
    ...
    const toast = await screen.findByRole('alert')
    expect(toast).toHaveTextContent('无权限')
