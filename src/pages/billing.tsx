...
function invoiceTypeLabel(type: string, options: BillingInvoiceOption[] = []): string {
  const label = options.find((option) => option.value === type)?.label
  if (label) return label
  if (type === 'special') return i18n.t('console.billing.invoiceTypeSpecial')
  if (type === 'normal') return i18n.t('console.billing.invoiceTypeNormal')
  return type || i18n.t('console.billing.statusUnknown')
}
  ...
  return <section className="analysis-section billing-ledger-section" aria-labelledby="billingLedgerHeading">
    <div className="section-heading">
      ...
      <div className="ledger-toolbar">
        ...
        <Button className="billing-export-button" title={!data?.items.length ? i18n.t('console.billing.noLedgerExport') : undefined} theme="outline" size="small" icon={<IconDownload />} loading={exporting} disabled={!data?.items.length || loading || exporting} onClick={onExport}>{i18n.t('console.billing.exportCsv')}</Button>
      </div>
    </div>
...
function CreditDetailsSection({ data }: { data: BillingAnalysisResponse }) {
  ...
  const rows: BillingQuotaRow[] = [
    ...
    { key: 'recharge', label: i18n.t('console.billing.quotaRecharge'), value: hasWalletQuotaDetails ? (wallet.recharge_amount_yuan ?? wallet.paid_available_yuan ?? '0') : (quota?.recharge_yuan ?? wallet.paid_available_yuan ?? '0'), icon: <IconCoinMoneyStroked aria-hidden="true" />, tone: 'recharge' },
...
function InvoiceTab({ state, invoiceTypes, invoiceInfoLoading, downloadingInvoiceID, onRetry, onOpenDialog, onDownload, onPageChange, onPageSizeChange, page, pageSize }: { state: ResourceState<BillingInvoiceResponse>; invoiceTypes: BillingInvoiceOption[]; invoiceInfoLoading: boolean; downloadingInvoiceID: string | null; onRetry: () => void; onOpenDialog: () => void; onDownload: (item: BillingInvoiceItem) => void; onPageChange: (page: number) => void; onPageSizeChange: (pageSize: number) => void; page: number; pageSize: number }) {
  ...
  const data = state.data
  return <section id="invoiceSection" className="invoice-page" aria-labelledby="invoiceHeading" tabIndex={-1}><h2 id="invoiceHeading" className="sr-only">{i18n.t('console.billing.invoice')}</h2><div className="invoice-faq"><h3 className="invoice-faq-title">{i18n.t('console.billing.invoiceFaq')}</h3><div className="invoice-faq-body">{i18n.t('console.billing.invoiceFaqHint')}</div></div><div id="invoiceOverview" data-invoice-view="overview"><div className="invoice-metrics" aria-label={i18n.t('console.billing.invoiceOverview')}><Metric label={i18n.t('console.billing.availableAmount')} value={<MoneyText value={data.available_amount_yuan} />} tone="invoice-metric-primary" action={<Button className="invoice-metric-action" theme="solid" type="primary" size="small" onClick={onOpenDialog} disabled={invoiceInfoLoading || isZeroYuan(data.available_amount_yuan)}>{i18n.t('console.billing.invoiceNow')}</Button>} /><Metric label={i18n.t('console.billing.issued')} value={formatCount(data.issued_count ?? 0)} note={i18n.t('console.billing.issuedDone')} /><Metric label={i18n.t('console.billing.issuing')} value={formatCount(data.pending_count ?? 0)} note={i18n.t('console.billing.waiting')} /></div><InvoiceHistory response={data} invoiceTypes={invoiceTypes} downloadingInvoiceID={downloadingInvoiceID} onDownload={onDownload} /><BillingPagination page={page} total={data.history.total} pageSize={data.history.page_size || pageSize} label={i18n.t('console.billing.invoiceHistory')} disabled={state.status !== 'success'} onPageChange={onPageChange} onPageSizeChange={onPageSizeChange} /></div></section>
}
