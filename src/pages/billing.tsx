...
function BillingPagination({ page, total, pageSize, label, disabled, onPageChange }: { page: number; total: number; pageSize: number; label: string; disabled: boolean; onPageChange: (page: number) => void }) {
  if (total <= pageSize) return null
  return <div className="billing-pagination"><span>{i18n.t('console.billing.pagination', { page, total: formatCount(total) })}</span><nav aria-label={label}><Pagination total={total} pageSize={pageSize} currentPage={page} hideOnSinglePage disabled={disabled} onPageChange={onPageChange} /></nav></div>
}

function LedgerTable({ items }: { items: BillingLedgerItem[] }) {
  return <div className="source-table-scroll billing-ledger-scroll" role="region" aria-label={i18n.t('console.billing.ledgerTable')} tabIndex={0}><table className="ledger-table"><thead><tr><th>{i18n.t('console.billing.time')}</th><th>{i18n.t('console.billing.type')}</th><th>{i18n.t('console.billing.source')}</th><th>{i18n.t('console.billing.relatedDescription')}</th><th>{i18n.t('console.billing.amountChange')}</th><th>{i18n.t('console.billing.balance')}</th></tr></thead><tbody>{items.map((item) => <tr key={`${item.kind}-${item.id}`}><td>{formatApiTime(item.occurred_at)}</td><td>{ledgerKindLabel(item.kind)}</td><td>{item.channel || '--'}</td><td><strong>{item.description || '--'}</strong>{item.request_id && item.kind !== 'reward' ? <Link to={`/console/records?request=${encodeURIComponent(item.request_id)}`}>{i18n.t('console.billing.viewRequest')}</Link> : null}{ledgerReference(item) ? <small>{ledgerReference(item)}</small> : null}</td><td className={item.direction === 'income' ? 'amount-positive' : 'amount-negative'}><MoneyText value={item.amount_yuan} direction={item.direction} /></td><td>{item.balance_after_yuan ? <MoneyText value={item.balance_after_yuan} /> : '--'}</td></tr>)}</tbody></table></div>
}
...
function RequestFocus({ data, requestId }: { data: BillingAnalysisResponse | null; requestId: string }) {
  ...
  return <div className="callout request-focus" aria-live="polite"><strong>{entry ? i18n.t('console.billing.requestSummary', { requestId }) : i18n.t('console.billing.requestNotFound', { requestId })}</strong><span>{entry ? <>{entry.description} · {entry.direction === 'expense' ? i18n.t('console.common.success') : i18n.t('console.billing.notBilled')} · {entry.direction === 'expense' ? <>{i18n.t('console.billing.cost')} <MoneyText value={entry.amount_yuan} /></> : i18n.t('console.billing.notBilled')}</> : i18n.t('console.billing.cleanedRequest')}</span><div className="request-focus-actions"><Link className="btn btn-secondary btn-sm" to="/console/billing">{i18n.t('console.billing.allLedger')}</Link>{entry ? <Link className="btn btn-secondary btn-sm" to={`/console/records?request=${encodeURIComponent(requestId)}`}>{i18n.t('console.billing.viewRecords')}</Link> : null}</div></div>
}

function AnalysisTab({ state, periodValue, apiKeyID, model, source, requestedRecordId, onRecharge, onSubscription, onInvoice, onSourceChange, onFilterChange, onPageChange, page, onRetry, onExport }: { state: ResourceState<BillingAnalysisResponse>; periodValue: string; apiKeyID: string; model: string; source: BillingSource; requestedRecordId: string; onRecharge: () => void; onSubscription: () => void; onInvoice: () => void; onSourceChange: (source: BillingSource) => void; onFilterChange: (key: 'period' | 'apiKey' | 'model', value: string) => void; onPageChange: (page: number) => void; page: number; onRetry: () => void; onExport: () => void }) {
...
function InvoiceDialog({ open, available, form, errors, step, submitting, onClose, onChange, onNext, onBack, onSubmit }: { open: boolean; available: string; form: InvoiceForm; errors: InvoiceFormErrors; step: 1 | 2 | 3; submitting: boolean; onClose: () => void; onChange: (key: keyof InvoiceForm, value: string) => void; onNext: () => void; onBack: () => void; onSubmit: () => void }) {
  ...
  return <Modal visible={open} title={step === 1 ? i18n.t('console.billing.applyInvoice') : step === 2 ? i18n.t('console.billing.confirmInvoice') : i18n.t('console.billing.invoiceSuccess')} onCancel={onClose} footer={null} className="invoice-dialog"><div className="invoice-steps"><span className={step >= 1 ? 'active' : ''}>1 {i18n.t('console.billing.fillStep')}</span><i /><span className={step >= 2 ? 'active' : ''}>2 {i18n.t('console.billing.confirmStep')}</span><i /><span className={step >= 3 ? 'active' : ''}>3 {i18n.t('console.billing.doneStep')}</span></div>{step === 1 ? <div className="dialog-body"><div className="invoice-form-grid"><label className={fieldClass('title')}><span className="invoice-field-label">{i18n.t('console.billing.invoiceTitle')} <em>*</em></span><input id="invoice-title" className="input" value={form.title} onChange={(event) => onChange('title', event.target.value)} required placeholder={i18n.t('console.billing.invoiceTitleRequired')} aria-invalid={Boolean(errors.title)} aria-describedby="invoice-title-error" />{fieldError('title')}</label><label className={fieldClass('tax_identifier')}><span className="invoice-field-label">{i18n.t('console.billing.taxpayerId')}{form.taxpayer_type === 'enterprise' ? <em>*</em> : null}</span><input id="invoice-tax-identifier" className="input" value={form.tax_identifier} onChange={(event) => onChange('tax_identifier', event.target.value)} placeholder={i18n.t('console.billing.taxpayerIdPlaceholder')} aria-invalid={Boolean(errors.tax_identifier)} aria-describedby="invoice-tax_identifier-error" />{fieldError('tax_identifier')}</label><label className={fieldClass('taxpayer_type')}><span className="invoice-field-label">{i18n.t('console.billing.taxpayerType')}</span><select id="invoice-taxpayer-type" className="input" value={form.taxpayer_type} onChange={(event) => onChange('taxpayer_type', event.target.value)} aria-invalid={Boolean(errors.taxpayer_type)} aria-describedby="invoice-taxpayer_type-error"><option value="enterprise">{i18n.t('console.billing.enterprise')}</option><option value="personal">{i18n.t('console.billing.personal')}</option></select>{fieldError('taxpayer_type')}</label><label className={fieldClass('invoice_type')}><span className="invoice-field-label">{i18n.t('console.billing.invoiceType')}</span><select id="invoice-type" className="input" value={form.invoice_type} onChange={(event) => onChange('invoice_type', event.target.value)} aria-invalid={Boolean(errors.invoice_type)} aria-describedby="invoice-invoice_type-error"><option value="special">{invoiceTypeLabel('special')}</option><option value="normal">{invoiceTypeLabel('normal')}</option></select>{fieldError('invoice_type')}</label><label className={fieldClass('project_name')}><span className="invoice-field-label">{i18n.t('console.billing.projectName')}</span><input id="invoice-project-name" className="input" value={form.project_name} readOnly aria-invalid={Boolean(errors.project_name)} aria-describedby="invoice-project_name-error" />{fieldError('project_name')}<small className="invoice-field-note">{i18n.t('console.billing.projectNameHint')}</small></label><label className={fieldClass('amount_yuan')}><span className="invoice-field-label">{i18n.t('console.billing.invoiceAmountYuan')} <em>*</em></span><input id="invoice-amount" className="input" inputMode="decimal" type="number" min="0.01" max={available} step="0.01" value={form.amount_yuan} onChange={(event) => onChange('amount_yuan', event.target.value)} placeholder={i18n.t('console.billing.amountMax', { amount: formatYuan(available) })} required aria-invalid={Boolean(errors.amount_yuan)} aria-describedby="invoice-amount_yuan-error" />{fieldError('amount_yuan')}</label><div className={fieldClass('email') + ' invoice-field-wide'}><label className="invoice-field-label" htmlFor="invoice-email">{i18n.t('console.billing.receivingEmail')} <em>*</em></label><input id="invoice-email" className="input" type="email" value={form.email} onChange={(event) => onChange('email', event.target.value)} placeholder={i18n.t('console.billing.accountEmail')} required aria-invalid={Boolean(errors.email)} aria-describedby="invoice-email-error" />{fieldError('email')}<small className="invoice-field-note">{i18n.t('console.billing.emailHint')}</small></div></div><p className="invoice-demo-note">{i18n.t('console.billing.invoiceSubmitDemo')}</p><div className="dialog-foot"><Link className="invoice-enterprise-link" to="/console/enterprise-create">{i18n.t('console.billing.enterpriseVerification')}</Link><Button theme="borderless" onClick={onClose}>{i18n.t('console.common.cancel')}</Button><Button theme="solid" type="primary" onClick={onNext}>{i18n.t('console.billing.confirmInvoice')}</Button></div></div> : step === 2 ? <div className="dialog-body"><p className="invoice-dialog-note">{i18n.t('console.billing.confirmInvoiceInfo')}</p><dl className="invoice-confirm-grid"><dt>{i18n.t('console.billing.invoiceTitle')}</dt><dd>{form.title}</dd><dt>{i18n.t('console.billing.taxpayerType')}</dt><dd>{form.taxpayer_type === 'enterprise' ? i18n.t('console.billing.enterprise') : i18n.t('console.billing.personal')}</dd><dt>{i18n.t('console.billing.invoiceType')}</dt><dd>{invoiceTypeLabel(form.invoice_type)}</dd><dt>{i18n.t('console.billing.invoiceAmount')}</dt><dd><MoneyText value={form.amount_yuan} /></dd><dt>{i18n.t('console.billing.receivingEmail')}</dt><dd>{form.email}</dd></dl><div className="dialog-foot"><Button theme="borderless" onClick={onBack} disabled={submitting}>{i18n.t('console.billing.checkAgain')}</Button><Button theme="solid" type="primary" loading={submitting} onClick={onSubmit}>{i18n.t('console.billing.confirmSubmit')}</Button></div></div> : <div className="dialog-body invoice-success"><div className="invoice-success-mark" aria-hidden="true"><IconTickCircle /></div><p className="invoice-dialog-note">{i18n.t('console.billing.invoiceSuccessHint')}</p><p className="invoice-demo-note">{i18n.t('console.billing.invoiceSuccessDemo')}</p><div className="dialog-foot"><Button theme="solid" type="primary" onClick={onClose}>{i18n.t('console.common.confirm')}</Button></div></div>}</Modal>
}

function InvoiceTab({ state, faqOpen, downloadingInvoiceID, onToggleFaq, onRetry, onOpenDialog, onDownload, onPageChange, page }: { state: ResourceState<BillingInvoiceResponse>; faqOpen: boolean; downloadingInvoiceID: string | null; onToggleFaq: () => void; onRetry: () => void; onOpenDialog: () => void; onDownload: (item: BillingInvoiceItem) => void; onPageChange: (page: number) => void; page: number }) {
...
export function BillingPage() {
  ...
  useEffect(() => {
    ...
    void getBillingAnalysis(context, { period, api_key_id: apiKeyID || undefined, model: model || undefined, source, page: analysisPage, page_size: BILLING_PAGE_SIZE, signal: controller.signal }).then((data) => {
      if (controller.signal.aborted) return
    ...
    return () => controller.abort()
  }, [analysisPage, apiKeyID, context, handleAuthFailure, loadError, model, period, reloadToken, source])

  useEffect(() => {
    if (activeTab !== 'invoice') return
    const controller = new AbortController()
    setInvoiceState((previous) => ({ ...resourceState('loading'), data: previous.data }))
    void getBillingInvoices(context, { page: invoicePage, page_size: BILLING_PAGE_SIZE, signal: controller.signal }).then((data) => {
    ...
    return () => controller.abort()
  }, [activeTab, context, handleAuthFailure, invoicePage, loadError, reloadToken])
