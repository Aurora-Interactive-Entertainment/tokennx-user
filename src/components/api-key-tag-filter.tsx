...
export function ApiKeyTagFilter({ tags, value, onChange }: { tags: string[]; value: string; onChange: (tag: string) => void }) {
  ...
  const labelID = useId()
  return <div className="api-key-tag-filter-control"><Select<string> className="api-key-tag-filter" aria-labelledby={labelID} value={value} filter onChange={(next) => onChange(typeof next === 'string' ? next : '')}>
