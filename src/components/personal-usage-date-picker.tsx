...
export function dateRangeToUtcBounds(range: Date[]): {
  ...
  const selectedStartAt = utcStartOfLocalDate(start);
  // 本地今天可能领先 UTC 一天；凌晨此时沿用服务端 today 的 UTC 今日起点，历史日期仍保持原 UTC 口径。
  const startAt = selectedStartAt > now ? utcToday : selectedStartAt;
  const nextDay = utcStartOfLocalDate(addLocalDays(end, 1));
  return { startAt, endAt: Math.min(now, nextDay) };
}
...
export function PersonalUsageDatePicker({
  ...
  return (
    <ConfigProvider
      locale={i18n.language.startsWith("en") ? semiEnUS : semiZhCN}
    >
      <DatePicker
