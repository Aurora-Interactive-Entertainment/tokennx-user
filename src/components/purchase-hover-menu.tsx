...
export function PurchaseHoverMenu({ onSelect }: { onSelect: () => void }) {
  ...
  return <div ref={root} className="header-trial-entry" onMouseEnter={() => { suppressFocus.current = false; setOpen(true); }} onMouseLeave={() => setOpen(false)} onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget)) setOpen(false); }} onKeyDown={event => { if (event.key === "Escape") { event.stopPropagation(); suppressFocus.current = true; setOpen(false); trigger.current?.focus(); } }}>
    <button ref={trigger} className="header-trial-badge" type="button" aria-label={t("console.common.subscription")} aria-expanded={open} aria-controls={id} onFocus={() => { if (!suppressFocus.current) setOpen(true); }} onClick={openCatalog}><img className="header-trial-pill" src={headerTrialPill} alt="" aria-hidden="true" /><span className="header-trial-glass" aria-hidden="true" /><strong>{t("console.common.trial")}</strong><span className="header-trial-subscription" aria-hidden="true">{t("console.common.subscription")}</span><span className="header-trial-free"><img src={headerTrialFreeTag} alt="" aria-hidden="true" /><em>{t("console.models.free")}</em></span></button>
