...
describe('目录真实峰谷定价', () => {
  ...
  it('切换英文后同步翻译时段信息、阶梯、单位和媒体说明', async () => {
    render(<ModelTimePricing model={model({
      ...
      pricing_periods: [period({ rules: [{ ...inputRule, quality_level: 'hd', duration_seconds: 5, tool_code: 'web_search' }] }), period({ key: 'fallback', default: true })],
    })} />)
    ...
    expect(screen.getByText(/Quality: hd/)).toHaveTextContent('Tier 1: 0 and above tokens')
