...
import {
  ...
  isEnterpriseOwner,
  type EnterpriseMenuAccess,
  type EnterpriseMenuPermissionKey,
  useEnterpriseMenuAccess,
} from "./enterprise-menu-access";
import {
  ENTERPRISE_CREATE_PATH,
...
const AUTHENTICATED_PUBLIC_LINK: PublicLink = {
  labelKey: "nav.billing",
  path: "/console/billing",
};
const BILLING_OVERVIEW_HOVER_DELAY_MS = 120;
const BILLING_OVERVIEW_CACHE_MS = 8_000;
const BILLING_BALANCE_VISIBILITY_STORAGE_KEY =
  "token-nx:billing-balance-visible:v1";
...
function billingOverviewContext(
...
function initialBillingBalanceVisible(): boolean {
  ...
  try {
    return (
      window.localStorage.getItem(BILLING_BALANCE_VISIBILITY_STORAGE_KEY) !==
      "hidden"
    );
  } catch {
    return true;
...
function persistBillingBalanceVisible(visible: boolean): void {
  try {
    window.localStorage.setItem(
      BILLING_BALANCE_VISIBILITY_STORAGE_KEY,
...
export function BrandLogo({
  ...
  return (
    <img
      className={`brand-logo-image brand-logo-image--${size}${className ? ` ${className}` : ""}`}
      src={tokenNxLogo}
      alt=""
      aria-hidden="true"
    />
...
export function AppLoadingScreen({ label }: { label?: string }) {
  ...
  return (
    <div className="app-loading-screen" role="status" aria-label={loadingLabel}>
      <div className="app-loading-screen__content">
        <span className="app-loading-screen__logo-shell" aria-hidden="true">
          <BrandLogo size="panel" />
        </span>
        <span className="public-sr-only">{loadingLabel}</span>
      </div>
    </div>
  );
}
...
function modelTagBaseColor(tag: ModelTagValue): string {
  ...
  if (/折扣|特价|discount|sale/.test(label))
    return MODEL_TAG_NAMED_COLORS.orange;
...
function modelTagStyle(tag: ModelTagValue): CSSProperties {
  ...
  const hex =
    compactHex.length === 3
      ? [...compactHex].map((value) => value.repeat(2)).join("")
      : compactHex;
  ...
  const normalizedSaturation = Math.round(
    Math.min(64, Math.max(38, saturation * 72)),
  );
...
function ModelColorTag({
  ...
  return (
    <span
      className={`model-color-tag${className ? ` ${className}` : ""}`}
      style={modelTagStyle(tag)}
    >
      {localizeConsoleLabel(t, tag.label)}
    </span>
  );
}
...
const MODEL_TYPE_MARKS: Record<string, string> = {
  文本: "T",
  图片: "I",
  视频: "V",
  音频: "A",
...
function modelParameterValue(
  model: ModelRecord,
  keys: readonly string[],
  suffix = "",
): string {
  const value = keys
    .map((key) => model.params?.[key])
...
function modelCardSpecs(model: ModelRecord): {
  contextLabel: string;
  contextValue: string;
  outputLabel: string;
  outputValue: string;
  ...
  if (model.modality === "image") {
    return {
      contextLabel: "图像尺寸",
      contextValue: modelParameterValue(model, ["尺寸", "sizes", "size"]),
      outputLabel: "输出限制",
  ...
  if (model.modality === "video") {
    return {
      contextLabel: "视频时长",
      contextValue: modelParameterValue(
        model,
        ["时长", "durations", "duration"],
        "s",
      ),
      outputLabel: "输出限制",
      outputValue: model.maxOutput ?? MODEL_UNAVAILABLE_LABEL,
  ...
  if (model.modality === "audio" || model.modality === "speech") {
    return {
      contextLabel: "音频音色",
      contextValue: modelParameterValue(model, ["音色", "voices", "voice"]),
      outputLabel: "输出限制",
      outputValue: model.maxOutput ?? MODEL_UNAVAILABLE_LABEL,
    };
  }
  if (model.modality === "transcription") {
    return {
      contextLabel: "音频格式",
      contextValue: modelParameterValue(model, [
        "音频格式",
        "formats",
        "format",
      ]),
...
export function ModelCard({
  ...
  const inputRaw =
    model.tokenNxPrice.input !== undefined
      ? model.tokenNxPrice.inputRaw
      : model.tokenNxPrice.standardRaw;
  const outputPrice = model.tokenNxPrice.output ?? model.tokenNxPrice.hd;
  const outputRaw =
    model.tokenNxPrice.output !== undefined
      ? model.tokenNxPrice.outputRaw
      : model.tokenNxPrice.hdRaw;
  const specs = modelCardSpecs(model);
  const inputMark =
    MODEL_TYPE_MARKS[inputTypeValue] ??
  ...
  const cardBody = (
    <>
      ...
      <div className="model-id-row">
        ...
        {copied ? (
          <span className="copy-hint">{t("console.common.copied")}</span>
        ) : null}
      </div>
      <div className="model-card-highlight-row">
        <span className="model-card-throughput">
          <strong>
            {model.throughput.unit === MODEL_UNAVAILABLE_LABEL
              ? t("console.common.unavailable")
              : `${model.throughput.value}${model.throughput.unit === "B tokens" ? "B token" : ` ${model.throughput.unit}`}`}
          </strong>
        </span>
      ...
      <div
        className="model-card-metrics"
        aria-label={t("console.modelDetail.informationTitle")}
      >
        <div>
          <span>
            {t("console.common.providers", { count: model.providerCount })}
          </span>
          <strong>{model.providerCount}</strong>
        </div>
        <div>
          <span>{t("console.modelDetail.platformTokens")}</span>
          <strong>
            {model.throughput.unit === MODEL_UNAVAILABLE_LABEL
              ? t("console.common.unavailable")
              : `${model.throughput.value}${model.throughput.unit === "B tokens" ? "B" : ` ${model.throughput.unit}`}`}
          </strong>
        </div>
        <div>
          <span>{t("home.rebuild.availability")}</span>
          <strong>
            {model.availability.rate > 0
      ...
      <div className="model-card-io">
        <div>
          <span className="model-card-io-label">
            {t("console.common.inputType")}{" "}
            <span className="model-card-type-mark" aria-hidden="true">
              {inputMark}
            </span>
          </span>
      ...
      <div
        className="model-card-pricing"
        aria-label={t("console.common.tokenNxPrice")}
      >
        <span className="model-card-price-caption">
          {t("console.common.tokenNxPriceHint")}
        </span>
        <div className="model-card-price-grid">
          {inputPrice !== undefined ? (
            <div className="model-card-price-cell">
              <span>{t("console.common.input")}:</span>
              <strong>
                <MoneyText
                  value={inputPrice}
                  rawValue={inputRaw}
          ...
          {outputPrice !== undefined ? (
            <div className="model-card-price-cell">
              <span>{t("console.common.output")}:</span>
              <strong>
                <MoneyText
                  value={outputPrice}
                  rawValue={outputRaw}
          ...
          {inputPrice === undefined && outputPrice === undefined ? (
            <div className="model-card-price-cell model-card-price-cell--single">
              <span>{t("console.common.tokenNxPrice")}:</span>
              <strong>
                <MoneyText
                  value={model.tokenNxPrice.base}
                  rawValue={model.tokenNxPrice.baseRaw}
                  withCurrency={false}
                />
                <small>{priceUnit}</small>
              </strong>
...
export function Sparkline({
  ...
  const points = values
    .map((value, index) => {
      ...
      const y =
        height - ((value - min) / Math.max(1, max - min)) * (height - 16) - 8;
      return `${x},${y}`;
    })
    .join(" ");
  return (
    <svg
      ...
      role="img"
      aria-label={t("console.common.trend")}
    >
...
function savePhoneCodeCooldown(destination: string, countryCode: string): void {
  try {
    window.localStorage.setItem(
      PHONE_CODE_COOLDOWN_KEY,
      JSON.stringify({
        destination,
        countryCode,
        expiresAt: Date.now() + LOGIN_CODE_RETRY_SECONDS * 1000,
      }),
    );
  } catch {
...
function remainingPhoneCodeCooldown(
  destination: string,
  countryCode: string,
): number {
  ...
  if (
    !cooldown ||
    cooldown.destination !== destination ||
    cooldown.countryCode !== countryCode
  )
    return 0;
...
function VerificationCodeButton(props: VerificationCodeButtonProps) {
  ...
  else if (props.retryAfter > 0)
    label = t("login.retryAfter", { seconds: props.retryAfter });
...
export function LoginPanel({
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(
      () => setPhoneRetryAfter((value) => Math.max(0, value - 1)),
      1000,
    );
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(
      () => setBindingRetryAfter((value) => Math.max(0, value - 1)),
      1000,
    );
    return () => window.clearInterval(timer);
  }, [bindingRetryAfter]);
  ...
  async function requestCode(): Promise<void> {
    const currentPhone = normalizeLoginPhone(
        ...
        <div className="login-pane login-pane--phone">
          <form className="login-form" onSubmit={submitPhone} noValidate>
            <h2 className="login-panel-title" id="login-panel-heading">
              <label htmlFor="login-phone">{t("login.phoneLoginTitle")}</label>
            </h2>
            <LoginPhoneField
              id="login-phone"
              phone={phone}
            ...
            <div className="form-field code-field">
              <label className="public-sr-only" htmlFor="login-code">
                {t("login.code")}
              </label>
              <div className="code-input-wrapper">
                <input
                  ...
                  value={code}
                  onChange={(event) =>
                    setCode(normalizeLoginPhone(event.target.value).slice(0, 6))
                  }
        ...
        >
          {wechatView === "binding" ? (
            <form className="login-form" onSubmit={submitBinding} noValidate>
              ...
              />
              <div className="form-field code-field">
                <label className="public-sr-only" htmlFor="binding-code">
                  {t("login.code")}
                </label>
                <div className="code-input-wrapper">
                  <input
                    ...
                    onChange={(event) =>
                      setBindingCode(
                        normalizeLoginPhone(event.target.value).slice(0, 6),
                      )
                    }
                    placeholder={t("login.codePlaceholder")}
                    maxLength={6}
                  ...
                  />
                  <VerificationCodeButton
                    loading={bindingCodeLoading}
                    retryAfter={bindingRetryAfter}
                    sent={bindingCodeSent}
                    onClick={() => {
                      void requestBindingCodeAction();
                    }}
                  />
                </div>
              </div>
              <div className="remember-checkbox">
                <input
                  ...
                  onChange={(event) => setRememberLogin(event.target.checked)}
                />
                <label htmlFor="binding-remember">
...
export function LoginDialog({
  ...
  return createPortal(
    <>
      <button
        className={`login-drawer-backdrop${open ? "" : " is-closing"}`}
        type="button"
        aria-label={t("login.close")}
        onClick={onClose}
      />
      <div
        className={`login-popover${open ? " is-open" : " is-closing"}`}
        id={dialogId}
        role="dialog"
        aria-modal="true"
        aria-label={t("login.dialogLabel")}
      >
        <button
...
export function LoginPopover({
  ...
  return (
    <div className="login-trigger-wrap">
      ...
      >
        <span className="header-login-label header-login-label--desktop">
          {t("login.trigger")}
        </span>
        <span className="header-login-label header-login-label--mobile">
          {t("login.loginRegister")}
        </span>
      </button>
      <LoginDialog
...
export function ThemeToggleButton() {
  ...
  return (
    <button
      className="header-tool theme-switcher"
      type="button"
      title={`${t("theme.switch")} · ${label}`}
...
function LanguageToggleButton({ mobile = false }: { mobile?: boolean }) {
  ...
  return (
    <button
      className={`header-tool language-switcher${isEnglish ? " is-english" : ""}${mobile ? " language-switcher--mobile" : ""}`}
      type="button"
    ...
    >
      <span className="language-switcher-track" aria-hidden="true">
        ...
        <span className="language-switcher-option language-switcher-option--en">
          EN
        </span>
        <span className="language-switcher-option language-switcher-option--zh">
          中
        </span>
      </span>
    </button>
...
export function PublicHeader({
  ...
  const [billingBalanceVisible, setBillingBalanceVisible] = useState(
    initialBillingBalanceVisible,
  );
  ...
  const billingOverviewRequestsRef = useRef(
    new Map<string, Promise<AccountOverviewResponse>>(),
  );
  const billingHoverRequestTimerRef = useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
  useEffect(() => {
    const handleNotificationCount = (event: Event) => {
      ...
      if (typeof count === "number" && Number.isFinite(count))
        setLiveUnreadNotificationCount(Math.max(0, Math.trunc(count)));
    };
    window.addEventListener(
      SUPPORT_NOTIFICATION_COUNT_EVENT,
  ...
  const billingBalance = formatBillingOverviewAmount(
    billingOverview?.account_balance_yuan,
  );
  const billingInvitationReward = formatBillingOverviewAmount(
    billingOverview?.invitation_reward_yuan,
  );
  const billingInvoiceableAmount = formatBillingOverviewAmount(
    billingOverview?.invoiceable_amount_yuan,
  );
  const maskedBillingBalance =
    billingBalance === "--"
      ? "--"
      : billingBalance.replace(/\D/g, "").replace(/\d/g, "*");
  ...
  function loadBillingOverview(
    ...
    void request
      .then((data) => {
        billingOverviewCacheRef.current.set(contextKey, {
          data,
          loadedAt: Date.now(),
        });
        if (billingContextCacheKeyRef.current === contextKey)
      ...
      .finally(() => {
        if (billingOverviewRequestsRef.current.get(contextKey) === request)
          billingOverviewRequestsRef.current.delete(contextKey);
      });
  }
  ...
  useEffect(() => {
    setBillingOverview(
      billingOverviewCacheRef.current.get(billingContextCacheKey)?.data ?? null,
    );
    ...
    const handlePointerDown = (event: PointerEvent): void => {
      if (
        ...
        event.target.closest(
          ".account-settings-overlay, .profile-contact-modal",
        )
      )
        return;
  ...
  function renderPublicLink(link: PublicLink, mobile = false): ReactNode {
    ...
    const isActive =
      ...
      (currentPath.startsWith(link.path) ||
        (link.labelKey === "nav.billing" &&
          currentPath.startsWith("/console/recharge")));
    ...
    const linkContent = (
      <>
        ...
        {mobile ? (
          <IconChevronRight
            className="public-mobile-nav-chevron"
            aria-hidden="true"
          />
        ) : null}
      </>
    ...
    const linkNode = (
      <Link
        ...
        onFocus={
          !mobile && link.labelKey === "nav.billing"
            ? openBillingMenu
            : undefined
        }
        aria-haspopup={
          !mobile && link.labelKey === "nav.billing" ? "dialog" : undefined
        }
      >
        {linkContent}
      </Link>
    ...
    return (
      <div
        className="public-billing-menu-shell"
        key={`${link.path}-${link.labelKey}`}
        onMouseEnter={openBillingMenu}
            ...
            <div className="billing-hover-card-top">
              <div className="billing-hover-balance">
                <div className="billing-hover-label">
                  <span>{t("console.billing.balance")}</span>
                  <span>{t("console.billing.currency")}</span>
                  <button
                    className="billing-hover-eye"
                    type="button"
                    aria-label={t(
                      billingBalanceVisible
                        ? "console.billing.hideBalance"
                        : "console.billing.showBalance",
                    )}
                    title={t(
                      billingBalanceVisible
                        ? "console.billing.hideBalance"
                    ...
                    onClick={toggleBillingBalanceVisibility}
                  >
                    {billingBalanceVisible ? (
                      <IconEyeOpenedStroked aria-hidden="true" />
                    ) : (
                      <IconEyeClosedStroked aria-hidden="true" />
                    )}
                  </button>
                </div>
                <strong
                  className={billingBalanceVisible ? "" : "is-hidden"}
                  title={
                    billingBalanceVisible
                      ? billingOverview?.account_balance_yuan
                      : undefined
              ...
              >
                <span>{t("console.billing.rechargeNow")}</span>
              </Link>
            </div>
            <div className="billing-hover-facts">
              <div>
                <span>{t("console.billing.promotionBalance")}</span>
                <strong title={billingOverview?.invitation_reward_yuan}>
                  {billingInvitationReward}
                </strong>
              </div>
              <div>
                <span>{t("console.billing.invoiceAvailable")}</span>
                <strong title={billingOverview?.invoiceable_amount_yuan}>
                  {billingInvoiceableAmount}
                </strong>
              </div>
            </div>
            <div className="billing-hover-divider" aria-hidden="true" />
            <Link
              className="billing-hover-center"
              to="/console/billing"
              onClick={() => setBillingMenuOpen(false)}
            >
  ...
  return (
    <>
      <header
        className={`app-header public-header public-header--home${scrolled ? " is-scrolled" : ""}${mobileOpen ? " mobile-nav-open" : ""}`}
        ref={headerRef}
      >
        <div className="app-header-inner app-header-full public-header-inner">
          ...
          <div className="header-actions public-header-actions">
            <div className="header-tools">
              <ThemeToggleButton />
              <button
                className="header-tool header-tool-badge"
                type="button"
                title={t("nav.notifications")}
                aria-label={t("nav.notifications")}
                onClick={() => requestSupportWidget("notifications")}
              >
                <IconBellStroked
        ...
        >
          ...
          <div className="public-mobile-tools">
            ...
            <button
              ...
              onClick={() => {
                setMobileOpen(false);
                requestSupportWidget("notifications");
              }}
            >
              <IconBellStroked
                className="header-notification-icon"
...
function ConsoleNavIcon({
  name,
  className = "",
}: {
  name: ConsoleNavIconName;
  className?: string;
}) {
  const Icon = CONSOLE_NAV_ICONS[name];
  return (
    <Icon
      className={`console-nav-icon${className ? ` ${className}` : ""}`}
      aria-hidden
    />
  );
}
...
const personalNavGroups: ConsoleNavGroup[] = [
  ...
  },
  {
    key: "analytics",
    label: "数据分析",
    items: [{ key: "/console/usage", label: "个人用量", icon: "usage" }],
  },
  {
    key: "account",
    label: "账户管理",
  ...
  },
  {
    ...
    items: [
      {
        key: ENTERPRISE_CREATE_PATH,
        path: NEW_ENTERPRISE_CREATE_PATH,
        label: "企业入驻",
        icon: "workspace",
      },
    ],
...
const enterpriseNavGroups: ConsoleNavGroup[] = [
  ...
  },
  {
    ...
    items: [
      {
        key: "/console/trae-enterprise/users",
        label: "人员管理",
        icon: "members",
        permissionScope: "members",
      },
      {
        key: "/console/trae-enterprise/usage",
        label: "用量管理",
        icon: "usage",
        permissionScope: "usage",
      },
      {
        key: "/console/trae-enterprise/operation-log",
        label: "操作日志",
        icon: "records",
        permissionScope: "audit",
      },
      {
        key: "/console/trae-enterprise/data-analysis",
        label: "数据分析",
        icon: "pie",
  ...
  },
  {
    ...
    items: [
      {
        key: "/console/enterprise-settings",
        label: "企业设置",
        icon: "settings",
      ...
      {
        key: "/console/enterprise-governance",
        label: "权限管理",
        icon: "governance",
...
const CONSOLE_NAV_LABEL_KEYS: Record<string, string> = {
  ...
  实名认证: "console.nav.realName",
  个人设置: "console.nav.profile",
  费用管理: "console.nav.billing",
  充值管理: "console.nav.recharge",
  "API 密钥管理": "console.nav.apiKeys",
  密钥管理: "console.nav.apiKeys",
  我的密钥: "console.nav.myApiKeys",
  企业中心: "console.nav.enterpriseCenter",
  企业入驻: "console.nav.enterpriseCreate",
  企业管理: "console.nav.enterpriseManagement",
  人员管理: "console.nav.members",
  用量管理: "console.nav.enterpriseUsage",
  操作日志: "console.nav.audit",
  ...
  权限管理: "console.nav.governance",
  账号信息: "console.nav.accountInfo",
  活动中心: "console.nav.activityCenter",
  邀请返现: "console.nav.invitationReward",
  认证返现: "console.nav.realNameReward",
};
...
export function consoleNavGroupsFor(
  ...
  const source =
    workspace.type === "enterprise" ? enterpriseNavGroups : personalNavGroups;
    ...
    },
    {
      ...
      paths:
        workspace.type === "enterprise"
          ? [
              "/console/trae-enterprise/users",
              "/console/usage",
              "/console/billing",
            ]
          : ["/console/usage", "/console/billing"],
    },
    {
      key: "profile-account",
      label: "账户管理",
      paths: ["/console/api-keys", "/console/invitations"],
    },
  ];
  return menuGroups
    .map((group) => ({
...
function workspaceRoleFromMembership(
...
export function workspacesFromMemberships(
  ...
  return memberships.reduce<Workspace[]>((workspaces, membership) => {
    ...
    workspaces.push({
      ...
      name,
      type: "enterprise",
      role: workspaceRoleFromMembership(membership),
    });
    return workspaces;
  }, []);
}
...
function UserMenu({
  ...
  const displayName =
    limitDisplayNameLength(userName.trim()) ||
    limitDisplayNameLength(store.nickname) ||
    t("console.common.demoUser");
  const phoneLabel = phone.trim() || t("console.common.phoneUnavailable");
  const initial = (displayName || store.avatar || "用")
    .slice(0, 1)
    .toUpperCase();
  const activeWorkspace = store.activeWorkspace;
  const groups = userMenuGroupsFor(
    activeWorkspace,
    enterpriseAccess?.permissions,
  );
  const keepMenuOpenForSettings =
    accountSettingsOpen &&
  ...
  useEffect(() => {
    ...
    void getProfileEnterprises(accessToken)
      .then((memberships) => {
        if (mounted)
          store.replaceEnterpriseWorkspaces(
            workspacesFromMemberships(memberships),
          );
      })
      .catch(() => {
  ...
  function updateWorkspaceMenuPosition(): void {
    ...
    const anchor = compactViewport
      ? trigger
      : (dropdownRef.current?.getBoundingClientRect() ?? trigger);
    const menu = workspaceMenuRef.current;
    if (!anchor || !menu) return;
    const menuWidth = menu.offsetWidth;
    const menuHeight = menu.offsetHeight;
    const maxLeft = Math.max(
      WORKSPACE_MENU_VIEWPORT_GAP_PX,
      window.innerWidth - menuWidth - WORKSPACE_MENU_VIEWPORT_GAP_PX,
    );
    const maxTop = Math.max(
      WORKSPACE_MENU_VIEWPORT_GAP_PX,
      window.innerHeight - menuHeight - WORKSPACE_MENU_VIEWPORT_GAP_PX,
    );
    const left = compactViewport
      ...
      : Math.min(
          Math.max(
            WORKSPACE_MENU_VIEWPORT_GAP_PX,
            anchor.left - menuWidth - WORKSPACE_MENU_GAP_PX,
          ),
          maxLeft,
        );
    const top = compactViewport
      ? Math.min(
          Math.max(WORKSPACE_MENU_VIEWPORT_GAP_PX, anchor.bottom + 8),
          maxTop,
        )
      ...
      >
        {store.workspaces.map((workspace) => {
          ...
          return (
            ...
            >
              ...
              {active ? (
                <IconTick
                  className="icon-svg workspace-menu-check"
                  aria-hidden="true"
                />
              ) : null}
            </button>
        ...
        {store.workspaces.length === 1 ? (
          <button
            ...
            type="button"
            role="menuitem"
            onClick={() => navigateFromMenu(NEW_ENTERPRISE_CREATE_PATH)}
          >
            <span className="workspace-avatar">+</span>
            <span className="workspace-info">
              <span className="workspace-name">
                {t("console.common.createWorkspace")}
              </span>
              <span className="workspace-type">
                {t("console.common.startEnterpriseVerification")}
              </span>
            </span>
          </button>
        ) : null}
  ...
  function renderMenuItem(item: ConsoleNavItem): ReactNode {
    ...
    if (!item.actionOnly) {
      ...
      return (
        ...
        >
          {icon}
          <span>{localizeConsoleNavLabel(t, item.label)}</span>
        </Link>
      );
    }
    return (
      <button
        className="dropdown-link dropdown-link-soon"
        role="menuitem"
        key={item.key}
        type="button"
        onClick={() => {
          ...
          Toast.info(
            item.soon
              ? t("console.common.comingSoon", {
                  name: localizeConsoleNavLabel(t, item.label),
                })
              : t("console.common.actionReceived"),
          );
        }}
      >
        {icon}
        ...
        {item.soon ? (
          <span className="nav-soon-badge">
            {t("console.common.comingSoonShort")}
          </span>
        ) : null}
      </button>
    );
  ...
  return (
    ...
    >
      <button
        ref={triggerRef}
        className="user-menu-trigger user-menu-trigger--avatar-only"
        type="button"
        aria-haspopup="menu"
        ...
        aria-label={
          open
            ? t("console.common.closeUserMenu")
            : t("console.common.openUserMenu")
        }
      ...
      <div
        ...
        id="user-dropdown"
        role="menu"
        aria-label={t("console.common.userMenu")}
        aria-hidden={!open}
        onMouseEnter={clearHoverCloseTimer}
        onMouseLeave={scheduleHoverClose}
      >
        <div className="user-dropdown-header">
          <button
            ...
            className="user-dropdown-identity"
            type="button"
            role="menuitem"
            aria-label={t("console.common.switchWorkspace")}
          ...
          >
            ...
            <span className="user-dropdown-identity-copy">
              <strong>{displayName}</strong>
              <span className="user-dropdown-workspace-line">
                <span>
                ...
                <span
                  className={`user-dropdown-identity-chevron${workspaceOpen ? " is-open" : ""}`}
                  aria-hidden="true"
                >
                  <IconChevronDown className="user-dropdown-identity-chevron-icon" />
                </span>
              </span>
              <span className="public-sr-only">{phoneLabel}</span>
              <span className="public-sr-only">
                {t("console.common.currentWorkspace")} ·{" "}
                {activeWorkspace.type === "personal"
                  ? displayName
                  : activeWorkspace.name}
              </span>
              <span className="public-sr-only">
                {t("console.common.switchWorkspace")}
              </span>
            </span>
          </button>
          <button
            className="user-dropdown-settings"
            type="button"
        ...
        {groups.map((group) => (
          <div className="user-dropdown-section" key={group.key}>
...
export function AccountSettingsModal({
  ...
  const fallbackDisplayName =
    limitDisplayNameLength(auth.user?.display_name || store.nickname) ||
    t("console.common.demoUser");
  ...
  const phone =
    profile?.phone.masked_identifier ||
    auth.user?.phone_masked ||
    store.phone ||
    t("profile.overview.notSet");
  const email =
    profile?.email.masked_identifier ||
    auth.user?.email_masked ||
    t("profile.overview.notSet");
  ...
  function applyProfile(nextProfile: UserProfile, publish = true): void {
    ...
    store.updateProfile({
      nickname: normalizedProfile.display_name,
      phone: normalizedProfile.phone.masked_identifier,
      avatar: normalizedProfile.avatar_url || store.avatar,
    });
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setLoadError("");
    void getUserProfile(accessToken)
      .then((nextProfile) => {
  ...
  async function commitName(): Promise<void> {
    ...
    if (!isValidDisplayName(value)) {
      appToast.warning(
        t("profile.personal.nameTooLong", {
  ...
  function renderNickname(): ReactNode {
    if (editingName) {
      return (
        <span className="account-settings-value account-settings-value--editing">
          <input
            className="account-settings-input"
            autoFocus
            value={editingValue}
            aria-label={t("profile.personal.nickname")}
            maxLength={PROFILE_DISPLAY_NAME_MAX_LENGTH}
            disabled={savingName}
            onChange={(event) =>
              setEditingValue(limitDisplayNameLength(event.target.value))
            }
            onBlur={() => {
              void commitName();
            }}
          ...
          <button
            type="button"
            className="account-settings-icon-button account-settings-confirm"
            aria-label={t("profile.personal.save")}
            disabled={savingName}
    ...
    return (
      <span className="account-settings-value">
        {displayName}
        <button
          type="button"
          className="account-settings-icon-button"
          aria-label={t("profile.personal.nickname")}
  ...
  return createPortal(
    <>
      ...
      >
        ...
        >
          <aside className="account-settings-sidebar">
            ...
            <button className="account-settings-tab active" type="button">
              <IconUserStroked aria-hidden="true" />
          ...
          <main className="account-settings-main">
            ...
            <div className="account-settings-fields">
              ...
              <div className="account-settings-row">
                ...
                <button
                  type="button"
                  className="account-settings-contact-trigger"
                  disabled={loading || !profile}
                  onClick={() => setContactProvider("email")}
                >
                  <span>{email}</span>
                  <span className="account-settings-inline-action">
                    {profile?.email.bound
                      ? t("profile.contact.changeEmail")
                      : t("profile.contact.dialogEmailBind")}
                  </span>
                </button>
              </div>
              <div className="account-settings-row">
                <span className="account-settings-label">
                  {t("profile.overview.id")}
                </span>
                <span className="account-settings-value">
                  {userId}
                  <button
              ...
              <div className="account-settings-row">
                ...
                <button
                  ...
                  aria-label={t("login.phone")}
                  disabled={loading || !profile}
                  onClick={() => setContactProvider("phone")}
                >
                  <span>{phone}</span>
                  <IconEditStroked aria-hidden="true" />
                </button>
              ...
              <div className="account-settings-row account-settings-row--badges">
                ...
                <span className="account-settings-badges">
      ...
      {contactProvider && profile ? (
        <Suspense fallback={null}>
          <LazyProfileContactDialog
            visible
            provider={contactProvider}
            currentContact={profile[contactProvider]}
            currentDestination={
              contactProvider === "phone"
                ? (getVerifiedPhone(profile.id) ?? undefined)
                : undefined
            }
...
function ConsoleTopBreadcrumb({
  ...
  const activeGroup = groups.find((group) =>
    group.items.some((item) => item.key === navKey),
  );
...
export function ConsoleLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const recordScrollPosition = () => {
      if (!modalVisible && !pendingScrollPosition)
        lastScrollPosition = { left: window.scrollX, top: window.scrollY };
    };
    const recordInteractionPosition = () => {
      if (!modalVisible)
        pendingScrollPosition = { left: window.scrollX, top: window.scrollY };
    };
    ...
    const hasVisibleModal = () =>
      Boolean(
        document.querySelector(
          ".semi-portal .semi-modal-wrap:not(.semi-modal-displayNone)",
        ),
      );
    const observer = new MutationObserver(() => {
      const nextModalVisible = hasVisibleModal();
      if (nextModalVisible && !modalVisible)
        restoreScrollPosition(pendingScrollPosition ?? lastScrollPosition);
  ...
  return (
    <div
      className={`console-frame public-header-host${sidebarOpen ? " console-frame--sidebar-open" : ""}${location.pathname === "/console/quickstart" ? " console-frame--quickstart" : ""}`}
    >
      ...
      <Layout className="console-layout">
        <Layout.Sider className="console-sider">
          <aside
            ...
            aria-label={t("console.common.consoleNav")}
          >
            <nav
              className="console-nav-custom"
              aria-label={t("console.common.consoleNav")}
            >
              {groups.map((group) => (
                <section className="console-nav-section" key={group.key}>
                  <h2 className="console-nav-section-title">
                    {localizeConsoleNavLabel(t, group.label)}
                  </h2>
                  <div className="console-nav-section-items">
                    {group.items.map((item) =>
                      item.actionOnly ? (
                        <button
                          className="console-nav-link console-nav-link--action"
                          key={item.key}
                          type="button"
                          onClick={() => {
                        ...
                        >
                          ...
                          <span className="console-nav-link-label">
                            {localizeConsoleNavLabel(t, item.label)}
                          </span>
                          {item.soon ? (
                            <span className="nav-soon-badge">
                              {t("console.common.comingSoonShort")}
                        ...
                        <Link
                          className={`console-nav-link${item.key === navKey ? " console-nav-link--active" : ""}`}
                          key={item.key}
                          to={item.path ?? item.key}
                          aria-current={
                            item.key === navKey ? "page" : undefined
                          }
            ...
            <div className="console-mobile-toolbar">
              ...
              <span>
                {localizeConsoleNavLabel(
                  ...
                  groups
                    .flatMap((group) => group.items)
                    .find((item) => item.key === navKey)?.label ??
                    t("nav.console"),
                )}
              </span>
            </div>
      ...
      {sidebarOpen ? (
        <button
          ...
          type="button"
          aria-label={t("console.common.closeConsoleNav")}
          onClick={() => setSidebarOpen(false)}
        />
      ) : null}
...
const MANUSCRIPT_FOOTER_GROUPS = [
  {
    ...
    mobileTitleKey: "footer.product",
    links: [
      {
        labelKey: "footer.chat",
        path: "/console/playground",
        requiresLogin: true,
      },
      { labelKey: "footer.video", path: "/console/video", requiresLogin: true },
      { labelKey: "footer.ranking", path: "/rankings" },
      { labelKey: "footer.agentRanking", path: "/apps" },
    ],
  },
  {
    titleKey: "footer.docs",
    mobileTitleKey: "footer.docs",
    links: [
      {
        labelKey: "footer.platformIntro",
        path: PUBLIC_FOOTER_DOC_HREFS.platformIntro,
      },
      { labelKey: "footer.userGuide", path: "/docs" },
      { labelKey: "footer.apiDocs", path: PUBLIC_FOOTER_DOC_HREFS.apiDocs },
      { labelKey: "footer.faq", path: PUBLIC_FOOTER_DOC_HREFS.faq },
    ],
  },
  {
    titleKey: "footer.pricing",
    mobileTitleKey: "footer.pricing",
    links: [
  ...
  },
  {
    ...
    mobileTitleKey: "footer.legal",
    links: [
      { labelKey: "footer.userAgreement", path: "/terms" },
      { labelKey: "footer.privacyAgreement", path: "/privacy" },
      { labelKey: "footer.rechargeAgreement", path: "/recharge-agreement" },
    ],
  },
] as const;
  ...
  return (
    <footer className="public-footer public-footer--manuscript">
      <div className="public-footer-inner">
        ...
        <nav
          className="public-footer-nav manuscript-footer-nav"
          aria-label={t("public.footer.navigation")}
        >
          {MANUSCRIPT_FOOTER_GROUPS.map((group) => {
            const isOpen = openManuscriptGroup === group.titleKey;
            const panelId = `manuscript-footer-panel-${group.titleKey.replace(/[^a-z0-9]+/gi, "-")}`;
            return (
              <div
                className={`public-footer-nav-group${isOpen ? " is-open" : ""}`}
                key={group.titleKey}
              >
                <strong className="manuscript-footer-group-title">
                  {t(group.titleKey)}
                </strong>
                <button
                  className="manuscript-footer-group-toggle"
                  type="button"
                  aria-expanded={isOpen}
                  aria-controls={panelId}
                  onClick={() =>
                    setOpenManuscriptGroup((current) =>
                      current === group.titleKey ? null : group.titleKey,
                    )
                  }
                >
                  <span
                    className="manuscript-footer-group-label"
                    data-mobile-label={t(group.mobileTitleKey)}
                  >
                    {t(group.titleKey)}
                  </span>
                  <span
                    className="manuscript-footer-group-symbol"
                    aria-hidden="true"
                  >
                    {isOpen ? "-" : "+"}
                  </span>
                </button>
                <div className="manuscript-footer-group-links" id={panelId}>
                  {group.links.map((link) =>
                    "requiresLogin" in link && link.requiresLogin ? (
                      <LoginRequiredAction
                        key={`${link.path}-${link.labelKey}`}
                        returnPath={link.path}
                      >
                        {t(link.labelKey)}
                      </LoginRequiredAction>
                    ) : (
        ...
        <div className="public-footer-contact manuscript-footer-contact">
          <strong>{t("footer.contact")}</strong>
          <div className="manuscript-footer-contact-line">
            <span>{t("footer.salesPrefix")}</span>
            <button
              className="manuscript-footer-online-action manuscript-footer-online-consultation"
              type="button"
              aria-label={`${t("footer.salesPrefix")}${t("footer.salesOnline")}`}
              onClick={() => requestSupportWidget("contact")}
            >
              {t("footer.salesOnline")}
            </button>
          </div>
          <div className="manuscript-footer-contact-line">
          ...
          <div className="manuscript-footer-qr-row">
            <div className="public-footer-qr">
              <img
                src={manuscriptCustomerQr}
                alt={t("footer.qrAlt")}
                loading="lazy"
                decoding="async"
              />
              <span>{t("footer.customerQr")}</span>
            </div>
            <div className="public-footer-qr">
              <img
                src={manuscriptOfficialQr}
                alt={t("footer.officialQr")}
                loading="lazy"
                decoding="async"
              />
      ...
      <div
        className="public-footer-bottom manuscript-footer-filing"
...
export function ManuscriptSupportWidget() {
  ...
  const [messages, setMessages] = useState<SupportChatMessage[]>(() => [
    { id: "support-welcome", role: "support", text: t("support.welcome") },
  ]);
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener(SUPPORT_OPEN_EVENT, handleOpenRequest);
  }, []);
  ...
  useEffect(() => {
    if (!mounted || tab !== "notifications") return undefined;
  ...
  function closePanel(): void {
    ...
    closeTimerRef.current = window.setTimeout(
      () => setMounted(false),
      MANUSCRIPT_SUPPORT_TRANSITION_MS,
    );
  }
  ...
  function sendMessage(): void {
    ...
    replyTimerRef.current = window.setTimeout(() => {
      setMessages((current) => [
        ...current,
        {
          id: nextMessageId("support"),
          role: "support",
          text: getMockSupportReply(text, supportLocale),
        },
      ]);
      setReplying(false);
  ...
  async function handleMarkAllNotificationsRead(): Promise<void> {
    ...
    try {
      ...
      publishNotificationCount(
        previousNotifications.filter((item) => !item.read).length,
      );
    ...
    >
      {mounted ? (
        <section
          ...
          role="dialog"
          aria-modal="false"
          aria-label={t("support.dialogLabel")}
        >
          <div
            className="manuscript-support-tabs"
            role="tablist"
            aria-label={t("support.panel")}
          >
            <span
              className={`manuscript-support-tab-thumb${tab === "notifications" ? " is-right" : ""}`}
              aria-hidden="true"
            />
            <button
              className={`manuscript-support-tab${tab === "contact" ? " is-active" : ""}`}
              type="button"
              role="tab"
            ...
            <button
              className={`manuscript-support-tab${tab === "notifications" ? " is-active" : ""}`}
              type="button"
              role="tab"
          ...
          {tab === "contact" ? (
            <div
              ...
              role="tabpanel"
              aria-label={t("support.contactTab")}
            >
              <header className="manuscript-support-chat-header">
                <span
                  className="manuscript-support-agent-avatar"
                  aria-hidden="true"
                >
                  <IconCustomerSupport />
                </span>
                <span className="manuscript-support-agent-meta">
                  <strong>{t("support.agentName")}</strong>
                  <span>
                    <i aria-hidden="true" />
                ...
                <button
                  className="manuscript-support-close"
                  type="button"
              ...
              <div
                className="manuscript-support-messages"
                role="log"
                aria-live="polite"
                aria-label={t("support.messageHistory")}
              >
                <p className="manuscript-support-chat-hint">
                  <IconCommentStroked aria-hidden="true" />
                  {t("support.welcomeHint")}
                </p>
                {messages.map((message) => (
                  <div
                    className={`manuscript-support-message${message.role === "user" ? " is-user" : " is-support"}`}
                    key={message.id}
                  >
                    {message.role === "support" ? (
                      <span
                        className="manuscript-support-message-avatar"
                        aria-hidden="true"
                      >
                        <IconCustomerSupport />
                      </span>
                    ) : null}
                    <p className="manuscript-support-message-bubble">
                      {message.text}
                    </p>
                  </div>
                ))}
                {replying ? (
                  <div
                    className="manuscript-support-message is-support"
                    role="status"
                  ...
                  >
                    ...
                    <span
                      className="manuscript-support-typing"
                      aria-hidden="true"
                    >
                      <i />
                      <i />
              ...
              <form
                className="manuscript-support-composer"
                onSubmit={handleComposerSubmit}
              >
                <label
                  className="public-sr-only"
                  htmlFor="manuscript-support-input"
                >
                  {t("support.inputLabel")}
                </label>
                <textarea
                  id="manuscript-support-input"
                  value={draft}
                  maxLength={MANUSCRIPT_SUPPORT_MESSAGE_MAX_LENGTH}
                  rows={2}
                  placeholder={t("support.inputPlaceholder")}
                  aria-label={t("support.inputLabel")}
                  onChange={(event) => setDraft(event.target.value)}
                  onKeyDown={handleComposerKeyDown}
                />
                <button
                  className="manuscript-support-send"
                  type="submit"
            ...
            <div
              className="manuscript-support-notifications"
              role="tabpanel"
            ...
            >
              {notificationLoading ? (
                ...
                <div className="manuscript-support-notifications-state">
                  <IconBellStroked />
                  <strong>{t("support.notificationsError")}</strong>
                </div>
              ) : notifications.length === 0 ? (
                <>
                  <IconBellStroked />
                  <strong>{t("support.noNotifications")}</strong>
                  <span>{t("support.noNotificationsHint")}</span>
                </>
              ) : (
...
export function PublicLayout({
  ...
  const manuscript =
    mainClassName.includes("home-page--manuscript") ||
    mainClassName.includes("docs-page--manuscript") ||
    mainClassName.includes("apps-page--manuscript") ||
    mainClassName.includes("rankings-page--manuscript") ||
    mainClassName.includes("news-page--manuscript");
  const modelCatalog = mainClassName.includes("public-models-page");
  const layoutClassName = manuscript
    ? " public-layout--manuscript-home"
    : modelCatalog
      ? " public-layout--models"
      : "";
  return (
    <div className={`public-layout public-header-host${layoutClassName}`}>
      <PublicHeader enterpriseAccess={enterpriseAccess} />
      <main
        className={`public-main${mainClassName ? ` ${mainClassName}` : ""}`}
      >
        {children}
      </main>
      <PublicFooter />
...
export function BannerNotice({
  ...
  if (!compact)
    return (
      <div className={`banner-notice banner-notice--${tone}`}>
        <span className="banner-notice-dot" />
        {children}
      </div>
    );
  const icon =
  ...
  return (
    <div
      className={`banner-notice banner-notice--${tone} banner-notice--compact`}
      role={tone === "warning" ? "alert" : "status"}
    >
      <span className="banner-notice-icon" aria-hidden="true">
        {icon}
      </span>
      <div className="banner-notice-compact-content">{children}</div>
    </div>
  );
}
...
export function WorkspaceBadge({ workspace }: { workspace: Workspace }) {
  return (
    <span className="workspace-badge">
      <Avatar
        size="extra-small"
        color={workspace.type === "enterprise" ? "blue" : "green"}
      >
...
export function SearchField({
  ...
  const resolvedPlaceholder =
    placeholder ?? t("console.common.searchModelPlaceholder");
  return (
...
export function RefreshButton({ onClick }: { onClick: () => void }) {
  ...
  return (
    <Button
      theme="borderless"
      icon={<IconRefresh />}
      aria-label={t("console.common.refreshData")}
      onClick={onClick}
    />
  );
}
...
export function StatusBadge({
  ...
  status: "success" | "failed" | "active" | "disabled" | "pending";
}) {
  const { t } = useTranslation();
  const labels = {
    success: t("console.common.success"),
    failed: t("console.common.failed"),
    active: t("console.common.enable"),
    disabled: t("console.common.disabled"),
