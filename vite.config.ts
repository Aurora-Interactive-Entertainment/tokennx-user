...
const CODE_SPLITTING_GROUPS = [
  ...
  {
    // Semi 的主题与结构样式按实际入口拆分，保持全局样式包体积稳定。
    name: 'semi-styles',
  ...
  {
    // Semi 组件按目录拆分，避免将全部控件和图标聚合到单个超大文件。
    ...
    priority: 20,
    // 按实际入口区分共用控件，避免首页因 Button/Modal 的共享依赖下载整套 Form。
    entriesAware: true,
    minSize: 10 * 1024,
  },
  {
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    build: {
      target: 'es2022',
      // 构建清单供首屏依赖预算检查使用，不包含源码或 Source Map。
      manifest: true,
      // 仅发布任务生成隐藏 Source Map，并在成功上传后删除本地产物。
      sourcemap: uploadSourceMaps ? 'hidden' : false,
      // 页面通过路由和运行时依赖分块，单个产物控制在 600KB 警戒线以内。
      ...
      rolldownOptions: {
        ...
        onwarn(warning, defaultHandler) {
          // 第三方 lottie-web 依赖 eval 实现表达式运行时，过滤其已知告警并保留其他告警。
